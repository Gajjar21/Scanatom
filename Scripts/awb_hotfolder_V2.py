...
try:
    from Scripts.centralized_audit import write_hotfolder_event as _ca_write_hotfolder
except Exception:
...
def process_pdf(pdf_path, awb_set, by_prefix, by_suffix,
    ...
    if _proceed_to_route:
        ...
        for k, v in rs.get('quarantine', {}).items():
            quarantine[k] = set(v)
        # ocr_cache is stored as [[key_list, value], ...] pairs — restore to {tuple: str}
    ...
    if not file_is_stable(pdf_path):
        # File is still being written — skip silently and let the watchdog re-enqueue
        # when the next on_modified event fires.  Do NOT write tracker/audit rows here:
        # the file hasn't been moved and will be processed normally once stable, which
        # would leave spurious NEEDS_REVIEW entries for files that ultimately match.
    ...
    # On third-pass resume _proceed_to_route=True, so the start row already exists.
    ...
    # Extracted at function-body level so it is available on both fresh and
    # third-pass resume paths (probe_texts and base_angle are restored from
    # resume_state on resume, so the extraction is safe either way).
    ...
    try:
        ...
        _captured = {
            ...
            # tuple keys round-trip correctly through JSON (repr→str breaks lookup).
            "ocr_cache":              [
    ...
    try:
        while True:
            ...
            try:
                ...
                # Immediate reload if main.py "Refresh DB" button dropped a trigger file
                if config.AWB_RELOAD_TRIGGER.exists():
                    try:
                        config.AWB_RELOAD_TRIGGER.unlink()
                    except Exception:
                        pass
                    try:
                        awb_set = load_awb_set_from_excel(AWB_EXCEL_PATH)
                        by_prefix, by_suffix = build_buckets(awb_set)
                        last_excel_mtime = AWB_EXCEL_PATH.stat().st_mtime
                        last_excel_load  = now
                        log(f"[DB] Forced reload: {len(awb_set)} AWBs loaded.")
                    except Exception as e:
                        log(f"[DB] Forced reload failed: {e}")

                # Refresh Excel (periodic mtime-based)
